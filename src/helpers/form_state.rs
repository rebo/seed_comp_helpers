// This is a partially complete and completely experimental form state custom hook
// only password and text inputs implemented
// this can be easily extendable to other other form element types

use comp_state::{clone_state, use_state};
use seed::prelude::*;
use std::sync::Arc;
    ...
    fn form_on_blur_event(&self) -> Option<seed::events::Listener<Ms>> {
        ...
        if closure.is_some() {
            Some(input_ev("blur", move |_text| {
                if let (Some(callback), Some(form_state)) = (closure, clone_state::<FormState>()) {
                    callback(form_state);
                };
                Ms::default()
            }))
